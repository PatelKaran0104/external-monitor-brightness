...
class BrightnessApp(tk.Tk):
    ...
    def _style(self) -> None:
        ...
        # Use a theme that honors custom colors across ttk controls.
    ...
    def _on_close_request(self) -> None:
        # Closing the window must NOT terminate the process — gamma ramps are
        # tied to this process and Windows resets them on exit. Hide to tray
        # instead and rely on the tray "Exit" item for an actual quit.
...
def acquire_single_instance() -> bool:
    # Returns True if this is the first instance. Otherwise surfaces the
    # already-running window and returns False so the caller can exit.
